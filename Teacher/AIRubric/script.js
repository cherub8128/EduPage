document.addEventListener("DOMContentLoaded", () => {
  ...
  byId("closeSidebarBtn").addEventListener("click", () =>
  ...
  function renderRubric() {
    ...
    state.rubric.forEach((item, index) => {
      ...
      summary.className =
        "p-3 flex items-center justify-between font-semibold cursor-pointer";
      summary.innerHTML = `
  ...
  function handleFileUpload(file, parser, successMsg) {
    ...
    reader.onload = (e) => {
      try {
        const newRubric = parser(e.target.result);
  ...
  byId("apply-pasted-rubric").addEventListener("click", () => {
    ...
    try {
      const newRubric = parseRubricCSV(text);
      if (newRubric && newRubric.length > 0) {
        state.rubric = newRubric;
        renderRubric();
        logln("SUCCESS", "텍스트로 루브릭을 적용했습니다.");
      } else {
        throw new Error("텍스트 내용이 비어있거나 형식이 잘못되었습니다.");
      }
    } catch (error) {
  ...
  byId("preset-save").addEventListener("click", () => {
    const blob = new Blob([JSON.stringify(state.rubric, null, 2)], {
  ...
  function parseRubricCSV(csvText) {
    ...
    lines.forEach((line) => {
      const [criterion, description, scoreStr] = line
        .split(",")
      ...
      rubricMap
        .get(criterion)
        .push({ description, score: parseFloat(scoreStr) });
    });

    const newRubric = [];
    for (const [name, levelsData] of rubricMap.entries()) {
      // 점수 기준으로 내림차순 정렬하여 A, B, C... 순서를 보장
      ...
      if (numLevels === 3) type = "3-point";
        ...
        logln(
          "WARN",
          `'${name}' 항목의 단계(${numLevels}개)가 비표준입니다. 단일 항목들로 처리합니다.`
        );
        levelsData.forEach((ld) => {
          newRubric.push({
            name: `${name}-${ld.description.slice(0, 10)}`,
            type: "single",
            scores: { 득점: ld.score },
  ...
  byId("startBtn").addEventListener("click", async () => {
    ...
    if (state.rubric.length === 0)
      return alert("평가 기준을 1개 이상 설정하세요.");
    ...
    logln(
      "INFO",
      `평가를 시작합니다. (Provider: ${opts.provider}, Model: ${opts.model})`
    );
    ...
    const rubricText = state.rubric
      .map((item) => {
        const descriptions = scoreTypeConfig[item.type].levels
          .map(
            (level) =>
              `  - ${level} (${item.scores[level]}점): ${item.descriptions[level]}`
          )
          .join("\n");
        return `- ${item.name}:\n${descriptions}`;
      })
    ...
    if (provider === "openai") {
      ...
      body = {
        ...
        messages: [
          {
            role: "system",
            content: "You are a strict rubric grader. Output JSON only.",
          },
  ...
  function renderSummary() {
    ...
    state.results.forEach((r) => {
      ...
      const scoreCells = state.rubric
        .map((item) => {
          const key = item.name.toLowerCase().replace(/\s+/g, "_");
          const level = r.scores?.[key] ?? "-";
          const score = item.scores[level] ?? 0;
          return `<td class="p-2 text-right">${score.toFixed(
            1
          )} (${level})</td>`;
        })
        .join("");
      tr.innerHTML = `<td class="p-2 truncate" title="${r.fileName}">${
        ...
        r.studentName || ""
      }</td>${scoreCells}<td class="p-2 text-right font-semibold">${meanScore.toFixed(
        2
      )}</td>`;
      body.appendChild(tr);
    });
  ...
  function renderCards(selectedFilename = null) {
    ...
    const maxPossibleScore = Math.max(
      ...state.rubric.map((item) => Math.max(...Object.values(item.scores))),
      1
    );
    ...
    card.innerHTML = `<div class="flex items-start justify-between gap-3"><div><h3 class="font-semibold text-lg">${
      ...
      r.studentId || "학번 미상"
    })</h3><p class="text-sm text-slate-500 truncate" title="${r.fileName}">${
      r.fileName
  ...
  function parseMetaFromFilename(name) {
    ...
    return m
      ? { studentId: m[1], studentName: m[2].trim() }
      : { studentId: "", studentName: base };
  }
  ...
  function calculateScores(scoresObj) {
    ...
    state.rubric.forEach((item) => {
      const key = item.name.toLowerCase().replace(/\s+/g, "_");
      const level = scoresObj[key];
      scoreValues.push(item.scores[level] || 0);
  ...
  byId("downloadCsv").addEventListener("click", () => {
    ...
    const rows = state.results.map((r) => {
      ...
      const scoreValues = state.rubric.flatMap((item) => {
        const key = item.name.toLowerCase().replace(/\s+/g, "_");
        const level = r.scores?.[key] ?? "-";
        const score = (item.scores[level] ?? 0).toFixed(1);
        return [score, level];
      });
      const sanitized = (str) =>
  ...
  function loadResults() {
    ...
    if (saved) {
      ...
      if (data.settings) {
        ...
        byId("model").value =
          data.settings.model || providers[byId("provider").value].defaultModel;
      }
      logln(
        "INFO",
        `${state.results.length}개의 저장된 평가 결과를 불러왔습니다.`
      );
    }
  }
